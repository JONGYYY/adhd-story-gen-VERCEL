...
async function main() {
  ...
  const perFontSeconds = Number(getArg('--per-font-seconds') || 1.5);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const vf =
      // Dark BG already provided by input; label uses default font to stay readable.
      `drawtext=text='${label}':fontsize=60:fontcolor=white:borderw=8:bordercolor=black:x=(w-text_w)/2:y=h*0.35,` +
      ...
      `drawtext=text='${String(i + 1)}/${String(files.length)}':fontsize=40:fontcolor=white@0.95:borderw=6:bordercolor=black:x=40:y=40`;
